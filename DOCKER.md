...
| Command                                                   | What starts                          |
| --------------------------------------------------------- | ------------------------------------ |
| `docker compose up -d`                                    | Pulse Music                           |
| `docker compose --profile pocketbase up -d`               | Pulse Music + PocketBase              |
| `docker compose --profile dev up -d`                      | Pulse Music + Dev server              |
| `docker compose --profile dev --profile pocketbase up -d` | Pulse Music + Dev server + PocketBase |

In `docker-compose.yml`, it looks like this:

```yaml
services:
    pulse music: # no profile -- always starts
...
Pulse Music uses Appwrite for user authentication. While it defaults to official instances, you can use your own self-hosted Appwrite instance:
...
3. Set these variables in your `.env`:
    ...
    - `APPWRITE_PROJECT_ID`: Your Appwrite project ID (e.g., `auth-for-pulse music`).

### Database (PocketBase)

Pulse Music uses PocketBase to store user data (playlists, favorites, profiles, etc.). You can run it alongside Pulse Music using the `pocketbase` profile:
