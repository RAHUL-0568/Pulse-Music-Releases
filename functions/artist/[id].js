...
class ServerAPI {
    ...
    async getInstances() {
        ...
        return [
            'https://eu-central.pulse music.tf',
            'https://us-west.pulse music.tf',
            'https://arran.pulse music.tf',
            'https://triton.squid.wtf',
            'https://api.pulse music.tf',
            'https://pulse music-api.samidy.com',
    ...
    if (isBot && artistId) {
        ...
        if (artist && (artist.name || artist.title)) {
            try {
                ...
                const description = `Listen to ${name} on Pulse Music`;
                const imageUrl = artist.picture
                    ? api.getArtistPictureUrl(artist.picture, '750')
                    : 'https://pulse music.samidy.com/assets/appicon.png';
