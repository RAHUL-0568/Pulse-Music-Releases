    ...
    if (isBot && sheetId && projectName) {
        try {
            ...
            if (artist && artist.name && trackerData && trackerData.eras) {
                ...
                const imageUrl = era && era.cover_art ? era.cover_art : 'https://pulse music.tf/assets/appicon.png';
                const pageUrl = new URL(request.url).href;
                const title = `${projectName} - ${artist.name}`;
                const description = `Stream ${projectName} by ${artist.name} on Pulse Music`;
