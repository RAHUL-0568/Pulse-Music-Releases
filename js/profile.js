...
async function uploadImage(file) {
    try {
        ...
        const response = await fetch(`https://worker.uploads.pulsemusic.qzz.io/${fileNameWithoutSpace}`, {
            method: 'PUT',
            headers: {
        ...
        return `https://images.pulsemusic.qzz.io/${await response.text()}`;
    } catch (error) {
...
export async function loadProfile(username) {
    ...
    if (isOwner) {
        ...
        document.getElementById('profile-avatar-container')?.classList.add('editable');
        document.getElementById('profile-banner')?.classList.add('editable');
    } else {
        document.getElementById('profile-avatar-container')?.classList.remove('editable');
        document.getElementById('profile-banner')?.classList.remove('editable');
    }
...
document.getElementById('profile-avatar-container')?.addEventListener('click', (e) => {
    if (e.currentTarget.classList.contains('editable')) {
        openEditProfile();
        // optionally trigger click on the file upload button automatically?
        // document.getElementById('edit-profile-avatar-upload-btn')?.click();
    }
});
