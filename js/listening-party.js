...
import { getTrackArtists, getTrackTitle, escapeHtml } from './utils.js';
...
export const LISTENING_PARTIES_DISABLED_MESSAGE =
    'Jam sessions are temporarily unavailable. Please try again later.';
...
export class ListeningPartyManager {
    ...
    async createParty() {
        ...
        const fallbackPartyName = ownerName ? `${ownerName}'s Party` : 'Jam Session';
        ...
        try {
            ...
            this.isHost = true;
            navigate(`/jam/${response.party.id}`);
        } catch (e) {
    ...
    async joinParty(partyId) {
        if (this.maintenanceMode) {
            await Modal.alert('Jam Sessions Unavailable', this.maintenanceMessage);
            navigate('/jams');
            return;
        }
    ...
    async showJoinModal(user, pbUser = null) {
        if (user && this._hasProfileName(pbUser)) {
            const confirmed = await Modal.confirm(
                'Join Party',
                `You are about to join a Jam Session. Everyone in the party will see your profile. Are you ready to listen together?`,
                'Join Party'
            );
    ...
    async _handleSocketMessage(message) {
        ...
        if (message.type === 'ended') {
            await Modal.alert('Party Ended', 'The host has ended the Jam Session.');
    ...
    updatePartyHeader() {
        ...
        if (metaEl) {
            const hostMember = (this.members || []).find((m) => m.is_host || m.isHost);
            const hostRecord = this.currentParty.expand?.host;
            const hostName =
                hostMember?.name?.trim() ||
                hostMember?.profile?.name?.trim() ||
                ...
                display.innerHTML = `
                    <div class="party-now-playing">
                        ...
                        <div class="party-now-playing-info">
                            <div class="party-now-playing-title">${escapeHtml(getTrackTitle(track))}</div>
            ...
            .map((r) => {
                try {
                    ...
                    <div class="track-info" style="flex: 1; min-width: 0;">
                        <div class="track-title" style="font-weight: 500; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${escapeHtml(getTrackTitle(r.track))}</div>
            ...
            list.querySelectorAll('.add-request-btn').forEach((btn) =>
                btn.addEventListener('click', async (e) => {
                    ...
                    if (req) {
                        ...
                        showNotification(`Added "${escapeHtml(getTrackTitle(req.track))}" to queue`);
    ...
    addChatMessage(msg) {
        ...
        if (!senderName && msg.memberId) {
            const member = this.members?.find((m) => m.id === msg.memberId);
        ...
        div.innerHTML = `
            <div style="font-weight: 600; font-size: 0.75rem; color: var(--primary); margin-bottom: 2px">${escapeHtml(senderName)}</div>
    ...
    async requestSong(track) {
        if (this.maintenanceMode) {
            showNotification('Jam sessions are temporarily unavailable');
            return;
        }
        if (!this.currentParty) return;
        try {
            ...
            showNotification(`Requested "${escapeHtml(getTrackTitle(track))}"`);
        } catch (e) {
        ...
        player.playTrackFromQueue = async (...args) => {
            if (this.currentParty && !this.isHost && !this.isInternalSync) {
                const leave = await Modal.confirm(
                    'Leave Jam?',
                    'Playing a song will cause you to leave the Jam session. Are you sure?',
                    'Leave and Play',
                    'danger'
                );
                if (!leave) return;
    ...
    copyInviteLink() {
        ...
        if (window?.Capacitor?.isNative || window?.Capacitor?.isNativePlatform?.()) {
            origin = 'https://pulsemusic.vercel.app';
        }
        const url = `${origin}/jam/${this.currentParty.id}`;
    ...
    showPartyIndicator() {
        ...
        btn.style.display = '';
        const name = this.currentParty.name || 'Jam Session';
        const count = this.members.length;
        const tooltip = `${name} (${count} listener${count === 1 ? '' : 's'})`;
        btn.title = tooltip;
        btn.setAttribute('aria-label', `Open Jam: ${tooltip}`);
        if (!btn.dataset.partyHandlerBound) {
            btn.addEventListener('click', () => {
                if (this.currentParty) navigate(`/jam/${this.currentParty.id}`);
            });
    ...
    hidePartyIndicator() {
        ...
        btn.setAttribute('aria-label', 'Open Jam');
    }
}
