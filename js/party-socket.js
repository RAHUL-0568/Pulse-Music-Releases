...
function getPartySocketUrl() {
    ...
    url.search = _PM_KEY ? `?key=${encodeURIComponent(_PM_KEY)}` : '';
    url.hash = '';
    return url.toString();
}
...
export class PartySocketClient {
    ...
    async connect() {
        ...
        this.connected = new Promise((resolve, reject) => {
            ...
            // Generous timeout: Render free tier can cold-start in 30-60s
            const connectTimeout = setTimeout(() => fail('Could not connect to the listening party server.'), 25000);
