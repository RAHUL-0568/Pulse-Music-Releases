...
function findBestMatch(items, targetTitle, targetArtist, targetAlbum, importOptions) {
    if (!items || items.length === 0) return null;
    if (!importOptions?.strictArtistMatch && !importOptions?.strictAlbumMatch && !targetTitle) return items[0];
    ...
    return (
        items.find((item) => {
            ...
            if (targetTitle && item.title) {
                if (!isFuzzyMatch(item.title, targetTitle)) titleOk = false;
            }
...
/**
 * Processes an array of track metadata concurrently with batching and exponential backoff
 */
async function processTracksConcurrently(trackMetadatas, api, onProgress, importOptions) {
    const tracks = [];
    const missingTracks = [];
    const BATCH_SIZE = 4;
    const MAX_RETRIES = 3;
    ...
    for (let i = 0; i < trackMetadatas.length; i += BATCH_SIZE) {
        const batch = trackMetadatas.slice(i, i + BATCH_SIZE);
        ...
        const batchPromises = batch.map(async (meta) => {
            ...
            for (let r = 0; r < MAX_RETRIES; r++) {
                try {
                    let searchQuery = `${title} ${artist}`.trim();
                    let searchResult = await api.searchTracks(searchQuery, { enrich: false });

                    // Fallback 1: Remove parentheses and brackets from title (e.g. "(feat. Artist)" or "[Remix]")
                    if (!searchResult || !searchResult.items || searchResult.items.length === 0) {
                        const cleanTitle = title.replace(/\s*\(.*?\)\s*/g, ' ').replace(/\s*\[.*?\]\s*/g, ' ').trim();
                        if (cleanTitle && cleanTitle !== title) {
                            searchQuery = `${cleanTitle} ${artist}`.trim();
                            searchResult = await api.searchTracks(searchQuery, { enrich: false });
                        }
                    }
                    
                    // Fallback 2: Search only by the clean title if artist combination fails
                    if (!searchResult || !searchResult.items || searchResult.items.length === 0) {
                        const cleanTitle = title.replace(/\s*\(.*?\)\s*/g, ' ').replace(/\s*\[.*?\]\s*/g, ' ').trim();
                        if (cleanTitle) {
                    ...
                    if (searchResult && searchResult.items && searchResult.items.length > 0) {
                        const match = findBestMatch(searchResult.items, title, artist, album, importOptions);
                        if (match) tracks.push(match);
                        else missingTracks.push(meta);
                    } else {
                        missingTracks.push(meta);
                    }
                    success = true;
                    break;
                } catch (e) {
                    console.warn(`Import failed for ${title}, retrying in ${delay}ms...`, e);
        ...
        // Wait a short delay between batches to be nice to the API
        if (i + BATCH_SIZE < trackMetadatas.length) {
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        try {
            if (itemType === 'track') {
                ...
                if (isrc) {
                    const searchResult = await api.searchTracks(`isrc:${isrc}`, { enrich: false });
                ...
                if (!foundTrack && trackName && artistName) {
                    let searchQuery = `${trackName} ${artistName}`.trim();
                    let searchResult = await api.searchTracks(searchQuery, { enrich: false });

                    if (!searchResult || !searchResult.items || searchResult.items.length === 0) {
                        const cleanTitle = trackName.replace(/\s*\(.*?\)\s*/g, ' ').replace(/\s*\[.*?\]\s*/g, ' ').trim();
                        if (cleanTitle && cleanTitle !== trackName) {
                            searchQuery = `${cleanTitle} ${artistName}`.trim();
                            searchResult = await api.searchTracks(searchQuery, { enrich: false });
                        }
                    }
                    ...
                    if (searchResult.items && searchResult.items.length > 0) {
                        foundTrack = findBestMatch(searchResult.items, trackName, artistName, albumName, options);
                    }
                }
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        if (values.length >= headers.length) {
            ...
            if (trackTitle && artistNames) {
                trackMetadatas.push({ title: trackTitle, artist: artistNames, album: albumName });
            }
        }
    }

    return await processTracksConcurrently(trackMetadatas, api, onProgress, importOptions);
}
    ...
    try {
        ...
        for (let i = 0; i < playlist.track.length; i++) {
            ...
            if (trackTitle && trackCreator) {
                trackMetadatas.push({ title: trackTitle, artist: trackCreator, album: trackAlbum });
            }
        }

        return await processTracksConcurrently(trackMetadatas, api, onProgress, {});
    } catch (error) {
...
export async function parseXSPF(xspfText, api, onProgress) {
    ...
    for (let i = 0; i < trackList.length; i++) {
        ...
        if (title && creator) {
            trackMetadatas.push({ title, artist: creator, album });
        }
    }
...
export async function parseXML(xmlText, api, onProgress) {
    ...
    for (let i = 0; i < trackElements.length; i++) {
        ...
        if (title && artist) {
            trackMetadatas.push({ title, artist, album });
        }
    }
...
export async function parseM3U(m3uText, api, onProgress) {
    ...
    for (let i = 0; i < trackInfo.length; i++) {
        ...
        if (info.title) {
            trackMetadatas.push({ title: info.title, artist: info.artist || '', album: '' });
        }
    }
