...
const POCKETBASE_URL = window.__POCKETBASE_URL__ || DEFAULT_POCKETBASE_URL;
...
const syncManager = {
    ...
    async _getUserRecord(uid) {
        ...
        const promise = (async () => {
            try {
                const record = await authApi('/api/sync');
                
                // Keep the fields the frontend expects
                const enrichedRecord = {
                    ...record,
                    id: uid,
                    firebase_id: uid,
                    username: record.username || uid,
                    display_name: record.username || uid,
                    avatar_url: record.avatar_url || null,
                    privacy: { playlists: 'public', lastfm: 'public' },
                };
                
                this._userRecordCache = enrichedRecord;
    ...
    async getUserData() {
        ...
        try {
            const playlistsArr = await authApi("/api/v1/playlists");
            playlistsArr.forEach(p => {
                customPlaylists[p.id] = p;
            });
        ...
        const userPlaylists = this._dedupeRecordMap(
            fetchedFromBackend ? customPlaylists : this.safeParseInternal(record.user_playlists, 'user_playlists', {}),
            'playlist'
        );
        ...
        const search_history = this.safeParseInternal(record.search_history, 'search_history', []);

        return { library, history, userPlaylists, userFolders, profile, search_history };
    },
    ...
    async _updateUserJSON(uid, field, data) {
        ...
        try {
            ...
            console.log(`[CloudSync] Successfully synced '${syncField}' to MongoDB.`);

            this._userRecordCache = {
                ...record,
                [field]: data, // just update local cache with what was passed
            };
        } catch (error) {
            console.error(`[CloudSync] Failed to sync ${field}:`, error);
        }
    },
    ...
    async syncUserPlaylist(playlist, action) {
        ...
        try {
            if (action === 'delete') {
                await fetch(`/api/v1/playlists/${playlist.id}`, {
                    method: 'DELETE',
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                
                // Update local cache
                if (this._userRecordCache && this._userRecordCache.user_playlists) {
                    delete this._userRecordCache.user_playlists[playlist.id];
                }
            } else {
                const payload = {
                    id: playlist.id,
                ...
                if (this._userRecordCache) {
                    if (!this._userRecordCache.user_playlists) this._userRecordCache.user_playlists = {};
                    this._userRecordCache.user_playlists[playlist.id] = payload;
                }
            }
        } catch (error) {
            console.error("Failed to sync playlist to custom backend", error);
        }
    },
        ...
        if (user) {
            ...
            try {
                ...
                if (cloudData) {
                    ...
                    // Sync search history
                    if (cloudData.search_history && Array.isArray(cloudData.search_history)) {
                        const localSearch = JSON.parse(localStorage.getItem('search-history') || '[]');
                        const mergedSearch = [...new Set([...localSearch, ...cloudData.search_history])].slice(0, 10);
                        if (JSON.stringify(localSearch) !== JSON.stringify(mergedSearch)) {
                            localStorage.setItem('search-history', JSON.stringify(mergedSearch));
                        }
                        if (JSON.stringify(cloudData.search_history) !== JSON.stringify(mergedSearch)) {
                            await this._updateUserJSON(user.$id, 'search_history', mergedSearch);
                        }
                    }
