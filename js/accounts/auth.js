...
const LEGACY_AUTH_TOKEN_KEY = 'pulse music-auth-token';
const NATIVE_OAUTH_HANDLED_URLS_KEY = 'pulse music-native-oauth-handled-urls';
const NATIVE_OAUTH_SCHEME = 'pulse music';
...
export class AuthManager {
        ...
        if (isCapacitorNative()) {
            try {
                ...
                GoogleAuth.initialize({
                    clientId: '217687474888-5151b2u1grl7a62omop5ui48995f4eto.apps.googleusercontent.com',
                ...
                if (!res.ok) {
                    ...
                    throw new Error("Backend verification failed: " + errText);
                }
                ...
                if (data.token) {
                    ...
                    this.user = data.user;
                    window.dispatchEvent(new CustomEvent('auth-state-changed', { detail: { user: this.user } }));
                    return { data: { user: this.user } };
                }
                throw new Error("No token returned from backend.");
            } catch (error) {
                console.error("Capacitor Google Auth error:", error);
                return { error: { message: "Native Google Login failed." } };
            }
        }
    ...
    async signInWithEmail(email, password) {
        try {
            ...
            this.updateUI(this.user);
            this.authListeners.forEach((listener) => listener(this.user));
            if (window.showToast) window.showToast(`Welcome back, ${this.user.username || 'User'}!`, 'welcome');
            return this.user;
        } catch (error) {
    ...
    async signUpWithEmail(email, password) {
        try {
            ...
            this.updateUI(this.user);
            this.authListeners.forEach((listener) => listener(this.user));
            if (window.showToast) window.showToast(`Welcome to Pulse Music, ${this.user.username || 'User'}!`, 'welcome');
            return this.user;
        } catch (error) {
    ...
    updateUI(user) {
        ...
        if (window.__AUTH_GATE__) {
            ...
            const so = document.getElementById('auth-signout-page-btn'); if(so) { so.style.display = 'flex'; so.onclick = () => this.signOut(); } const ea = document.getElementById('email-auth-container'); if (ea) ea.style.display = 'none';
        ...
        if (user) {
            ...
            const so = document.getElementById('auth-signout-page-btn'); if(so) so.style.display = 'none'; const ea = document.getElementById('email-auth-container'); if (ea) ea.style.display = 'flex';
