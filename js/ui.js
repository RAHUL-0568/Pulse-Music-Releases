...
export class UIRenderer {
        ...
        window.addEventListener('fullscreen-cover-settings-changed', () => {
            ...
            if (overlay && overlay.style.display === 'flex') {
                ...
                if (this.player?.currentTrack) {
                    this.updateFullscreenMetadata(this.player.currentTrack);
                }
            }
        });
    ...
    async extractAndApplyColor(url) {
        ...
        let targetUrl = url;
        if (targetUrl.includes('resources.tidal.com') || targetUrl.includes('http')) {
            // Use proxy if not already proxied to ensure CORS header is present for Canvas
            if (!targetUrl.includes('wsrv.nl') && !targetUrl.startsWith('/') && !targetUrl.startsWith('data:')) {
                targetUrl = `https://wsrv.nl/?url=${encodeURIComponent(targetUrl)}&w=160&output=webp`;
            }
        }

        // Check cache first
        if (this.vibrantColorCache.has(targetUrl)) {
            const cachedColor = this.vibrantColorCache.get(targetUrl);
            if (cachedColor) {
        ...
        img.src = `${targetUrl}${separator}not-from-cache-please`;
        ...
        list.innerHTML = pinnedItems
            .map((item) => {
                ...
                if (item.type === 'user-playlist' && !item.cover && item.images && item.images.length > 0) {
                    ...
                    iconHTML = `<img crossorigin="anonymous" referrerpolicy="no-referrer" src="${coverUrl}" class="pinned-item-cover ${coverClass}" alt="${escapeHtml(item.name)}" loading="lazy" onerror="this.src='/image.png'">`;
                }
    ...
    async updateFullscreenMetadata(track, nextTrack) {
        ...
        if (isRealVideo) {
            ...
            const isCdMode = visualizerSettings.isCdAlbumCoverEnabled();
            const rawVideoCoverUrl = track.videoUrl || track.videoCoverUrl || track.album?.videoCoverUrl || null;
            const videoCoverUrl = isCdMode ? null : rawVideoCoverUrl;
            const staticCoverUrl = this.api.getCoverUrl(track.album?.cover || track.cover || track.image, '1280');
            ...
            if (currentImage && currentImage.removeAttribute) {
    ...
    async checkPartiesServerAvailable() {
        // Render's free tier sleeps after idle and cold-starts in 30-60s, so a
        // single short probe would wrongly report Jam as unavailable. Retry with
        // a generous timeout to give the backend time to wake up.
        const attempts = 3;
        const perAttemptTimeout = 20000;
    ...
    async renderHomePage() {
        ...
        try {
            ...
            if (seeds.length === 0) {
                try {
                    const searchRes = await this.api.searchTracks("popular hits");
                    if (searchRes.items && searchRes.items.length > 0) {
                        seeds = searchRes.items.slice(0, 10);
                    }
                } catch (e) {
                    console.warn("Failed to fetch default seeds");
                }
            }
            // DO NOT await these, otherwise the router gets completely blocked if Tidal/Monochrome are rate limiting
            Promise.all([
        ...
        const loadLists = async (year) => {
            ...
            try {
                ...
                for (const entry of data.lists) {
                    ...
                    thumb.src = entry.cover ? 'https://wsrv.nl/?url=' + encodeURIComponent(entry.cover) : '/image.png';
                    thumb.width = 44;
                    thumb.height = 44;
                    thumb.loading = 'lazy';
                    thumb.onerror = () => {
        ...
        try {
            ...
            for (const item of items) {
                ...
                row.innerHTML = `
                    ...
                    <img crossorigin="anonymous" referrerpolicy="no-referrer" src="${item.cover ? 'https://wsrv.nl/?url=' + encodeURIComponent(item.cover) : '/image.png'}" width="48" height="48"
                        style="border-radius:6px;object-fit:cover;flex-shrink:0;background:var(--secondary);"
                        loading="lazy" onerror="this.src='/image.png';this.onerror=null;">
    ...
    createAOTYAlbumCardHTML(album, isUpcoming = false) {
        ...
        const coverUrl = album.cover ? "https://wsrv.nl/?url=" + encodeURIComponent(album.cover) : '/image.png';
        const cover = `<img referrerpolicy="no-referrer" src="${coverUrl}" alt="${title}" class="card-image" loading="lazy" onerror="this.src='/image.png'">`;
    ...
    async renderHomeAlbums(forceRefresh = false, providedSeeds = null, retryCount = 0) {
        ...
        if (albumsContainer) {
            ...
            try {
                ...
                // Iterate through seeds until we find one that successfully returns similar albums
                let filteredAlbums = [];
                for (const seed of seeds) {
                    if (seed.album && seed.album.id) {
                        try {
                            const similarAlbums = await this.api.getSimilarAlbums(seed.album.id);
                            filteredAlbums = await this.filterUserContent(similarAlbums, 'album');
                            if (filteredAlbums.length > 0) {
                                break; // Found good recommendations!
                            }
                        } catch (err) {
                            console.warn(`Failed to get similar albums for seed ${seed.album.id}, trying next...`, err.message);
                        }
                    }
                }
    ...
    async renderHomeArtists(forceRefresh = false, providedSeeds = null) {
        ...
        if (artistsContainer) {
            ...
            try {
                ...
                let filteredArtists = [];
                for (const seed of seeds) {
                    const artistId = seed.artist?.id || seed.artists?.[0]?.id;
                    if (artistId) {
                        try {
                            ...
                            console.warn(`Failed to get similar artists for seed ${artistId}, trying next...`, err.message);
                        }
                    }
                }
    ...
    renderSearchHistory() {
        ...
        if (clearBtn) {
            clearBtn.addEventListener('click', (e) => {
                ...
                if (authManager.user) {
                    syncManager._updateUserJSON(authManager.user.$id, 'search_history', []);
                }
            });
        }
    }
    ...
    removeFromSearchHistory(query) {
        ...
        this.renderSearchHistory();
        if (authManager.user) {
            syncManager._updateUserJSON(authManager.user.$id, 'search_history', history);
        }
    }
                        ...
                        rateCriticsEl.querySelector('a').onclick = () => {
                            ...
                            for (const review of reviews) {
                                ...
                                <img crossorigin="anonymous" src="${review.image || ''}" width="50" height="50" style="border-radius:8px;object-fit:cover;background:var(--highlight);flex-shrink:0;"
                                     onerror="this.src='/image.png';this.onerror=null;" loading="lazy" referrerpolicy="no-referrer">
                                <div style="flex:1;">
    ...
    async renderPlaylistPage(playlistId, source = null, _provider = null) {
        ...
        try {
            ...
            if (playlistData) {
                ...
                document.title = `${playlistData.name || playlistData.title} - Pulse Music`;
    ...
    async renderPodcastsBrowsePage() {
        ...
        document.title = 'Podcasts - Pulse Music Music';
    }
    ...
    async renderPodcastPage(podcastId) {
        ...
        try {
            ...
            document.title = `${podcastResult?.title || 'Podcast'} - Pulse Music Music`;
