...
class HiFiClient {
    ...
    static parseIsoDuration(iso: string | undefined): number | undefined {
    ...
    async getArtist(
        ...
        if (id) {
            ...
            const parseIsoDuration = HiFiClient.parseIsoDuration;
        ...
        if (i) {
            try {
                const res = await this.#fetchJson<SearchResponse['data']>(
                    ...
                    { 'filter[isrc]': i, limit, offset, countryCode: this.#countryCode },
                    signal
                );
                return HiFiClient.#jsonResponse({ version: HiFiClient.API_VERSION, data: res });
            } catch (err: unknown) {
            ...
            const fallback = await this.#fetchJson<any>(
                ...
                { query: i, types: 'TRACKS', limit, offset, countryCode: this.#countryCode },
                signal
            );
            return HiFiClient.#jsonResponse({ version: HiFiClient.API_VERSION, data: fallback });
        }

        const mapping: Array<[string | undefined, string, Record<string, string | number>]> = [
            [q, 'https://api.tidal.com/v1/search', { query: q || '', types: 'ARTISTS,ALBUMS,TRACKS,VIDEOS,PLAYLISTS', limit, offset, countryCode: this.#countryCode }],
            [s, 'https://api.tidal.com/v1/search', { query: s || '', types: 'TRACKS', limit, offset, countryCode: this.#countryCode }],
            [a, 'https://api.tidal.com/v1/search', { query: a || '', types: 'ARTISTS,TRACKS', limit, offset, countryCode: this.#countryCode }],
            [al, 'https://api.tidal.com/v1/search', { query: al || '', types: 'ALBUMS', limit, offset, countryCode: this.#countryCode }],
            [v, 'https://api.tidal.com/v1/search', { query: v || '', types: 'VIDEOS', limit, offset, countryCode: this.#countryCode }],
            [p, 'https://api.tidal.com/v1/search', { query: p || '', types: 'PLAYLISTS', limit, offset, countryCode: this.#countryCode }],
        ];
