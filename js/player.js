...
export class Player {
    ...
    async updateVideoCovers(videoUrl) {
        ...
        let isCdMode = false;
        try {
            isCdMode = localStorage.getItem('cd-album-cover') !== 'false';
        } catch {}
        if (fullscreenCover && !isCdMode) await syncCover(fullscreenCover);
    }
    ...
    async playTrackFromQueue(startTime = 0, recursiveCount = 0, isRetry = false, options = {}) {
        ...
        try {
            ...
            if (window.showToast) {
                window.showToast(`"${trackTitle}" is unavailable on all streaming services.`, 'error');
            }
        }
    }
    ...
    fetchRadioRecommendations() {
        ...
        this.radioFetchPromise = (async () => {
            try {
                ...
                let recommendations = [];
                try {
                    // Try to get specific track recommendations first (much better for single-song radio)
                    const trackPromises = seeds.map(seed => this.api.getTrackRecommendations(seed.id));
                    const trackRecsList = await Promise.all(trackPromises);
                    
                    for (const recs of trackRecsList) {
                        if (recs && Array.isArray(recs)) {
                            recommendations.push(...recs);
                        }
                    }
                    
                    // Fallback to playlist/artist-based if track specific failed
                    if (recommendations.length === 0) {
                        recommendations = await this.api.getRecommendedTracksForPlaylist(seeds, 20, {
                            knownTrackIds: knownTrackIds,
                        });
                    }
                } catch (e) {
                    console.error("Failed track recommendations, falling back", e);
                    recommendations = await this.api.getRecommendedTracksForPlaylist(seeds, 20, {
                        knownTrackIds: knownTrackIds,
                    });
                }
    ...
    fetchAutoplayRecommendations() {
        ...
        this.autoplayFetchPromise = (async () => {
            try {
                ...
                let recommendations = [];
                try {
                    const trackPromises = seeds.slice(0, 3).map((seed) => this.api.getTrackRecommendations(seed.id));
                    const trackRecsList = await Promise.all(trackPromises);
                    for (const recs of trackRecsList) {
                        if (recs && Array.isArray(recs)) {
                            recommendations.push(...recs);
                    ...
                    console.warn('Direct autoplay track recommendations failed, falling back:', e);
                }
