...
export async function initializeSettings(scrobbler, player, api, ui) {
    ...
    const emailInputPage = document.getElementById('auth-email-page');
    const passwordInputPage = document.getElementById('auth-password-page');
    
    const googleAuthPageBtn = document.getElementById('google-auth-page-btn');
    if (googleAuthPageBtn) {
        googleAuthPageBtn.addEventListener('click', () => {
            authManager.signInWithGoogle();
        });
    }

const signInPageBtn = document.getElementById('email-signin-page-btn');
    const signUpPageBtn = document.getElementById('email-signup-page-btn');

    if (signInPageBtn) {
        signInPageBtn.addEventListener('click', async () => {
            if (!emailInputPage.value || !passwordInputPage.value) return alert('Enter email and password');
            signInPageBtn.textContent = '...';
            try {
                await authManager.signInWithEmail(emailInputPage.value, passwordInputPage.value);
            } finally {
                signInPageBtn.textContent = 'Sign In';
            }
        });
    }

    if (signUpPageBtn) {
        signUpPageBtn.addEventListener('click', async () => {
            if (!emailInputPage.value || !passwordInputPage.value) return alert('Enter email and password');
            signUpPageBtn.textContent = '...';
            try {
                await authManager.signUpWithEmail(emailInputPage.value, passwordInputPage.value);
    ...
    const LEGACY_GEQ_CUSTOM_PRESETS_KEY = 'pulse music-legacy-geq-custom-presets';
    ...
    document.getElementById('export-library-btn')?.addEventListener('click', async () => {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `pulse music-library-${new Date().toISOString().split('T')[0]}.json`;
        a.click();
        URL.revokeObjectURL(url);
});
    ...
    document.getElementById('export-settings-btn')?.addEventListener('click', () => {
        ...
        for (let i = 0; i < localStorage.length; i++) {
            ...
            if (key && key.startsWith('pulse music-')) {
                try {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `pulse music-settings-${new Date().toISOString().split('T')[0]}.json`;
        a.click();
        URL.revokeObjectURL(url);
});
    ...
    if (customDbBtn && customDbModal) {
        ...
        customDbBtn.addEventListener('click', () => {
            const pbUrl = localStorage.getItem('pulse music-pocketbase-url') || '';
            const appwriteEndpoint = localStorage.getItem('pulse music-appwrite-endpoint') || '';
            const appwriteProject = localStorage.getItem('pulse music-appwrite-project') || '';
        ...
        customDbSaveBtn.addEventListener('click', () => {
            if (!pbFromEnv && customPbUrlInput) {
                ...
                if (pbUrl) {
                    localStorage.setItem('pulse music-pocketbase-url', pbUrl);
            } else {
                    localStorage.removeItem('pulse music-pocketbase-url');
            }
        }
            ...
            if (!appwriteFromEnv) {
                ...
                if (endpoint) {
                    localStorage.setItem('pulse music-appwrite-endpoint', endpoint);
            } else {
                    localStorage.removeItem('pulse music-appwrite-endpoint');
            }

                if (project) {
                    localStorage.setItem('pulse music-appwrite-project', project);
            } else {
                    localStorage.removeItem('pulse music-appwrite-project');
            }
        }
            ...
            if (
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            const deleteRequest = indexedDB.deleteDatabase('Pulse MusicDB');
